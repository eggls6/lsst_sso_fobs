...
!//////////////////////////////////////////////////////////////////////////////////
 ...
 public::gravfr     !gravitational focusing radius
...
!*********************************************** 
SUBROUTINE bplane(rvp,rva,b,bpcoo)
!Valsecchi

implicit none
real(kind=wp),intent(in)::rvp(1:6),rva(1:6) !planetary, asteroidal heliocentric position and velocity
real(kind=wp),intent(out)::b,bpcoo(1:3)

real(kind=wp),dimension(1:3)::px,py,pz !planetocentric frame
real(kind=wp),dimension(1:3,1:3)::A !matrix with planetocetric basis
real(kind=wp),dimension(1:3)::dr,dv,dra,dva
real(kind=wp),dimension(1:3)::xi

real(kind=wp)::phi,theta,cth,sth

integer::i

!planetocentric frame
px(1:3)=-rvp(1:3)/norm(rvp(1:3))
py(1:3)=rvp(4:6)/norm(rvp(4:6))

call crossp3d(px,py,pz)
...
!planet to asteroid distance and velocity vector
dv(1:3)=rva(4:6)-rvp(4:6)
dr(1:3)=rva(1:3)-rvp(1:3)

write(*,*)'dr',dr
write(*,*)'dv',dv

!transform relative velocity into planetocentric frame
call v2basis(A,dv,dva)
call v2basis(A,dr,dra)

write(*,*)'dra',dra
write(*,*)'dva',dva

!get xi axis direction U x Y = xi
call crossp3d(dv/norm(dv),py,xi)

write(*,*)'xi',xi

!get the angles for rotating into b-plane
phi=atan2(dva(1),dva(3))

write(*,*)'phi',phi*rad2deg

cth=(dva(2)/norm(dva))
sth=sqrt(1._wp-cth*cth)

write(*,*)'theta',atan2(sth,cth)*rad2deg

!rotate for -phi around Y axis
call roty(-phi,dra)
!rotate for -theta around xi axis
call rotax2(cth,-sth,xi,dra)

bpcoo=dra

b=sqrt(bpcoo(1)*bpcoo(1)+bpcoo(3)*bpcoo(3))
...
!***********************************************
SUBROUTINE gravfr(vinf2,ve2,r2,b2)
!------------------------------------------------------------------------------------------------------------
! calculates gravitational focusing radius using v_inf^2, v_esc^2 and planet radius squared r^2
! Be sure that units are coherent! Recommended : km^2 and (km/s)^2
!
...
!------------------------------------------------------------------------------------------------------------
implicit none
real(kind=wp),intent(in)::vinf2,ve2,r2
real(kind=wp),intent(out)::b2

b2=r2*1._wp+ve2/vinf2
return
end subroutine
...
SUBROUTINE rv2kema (rv,ele,m0,m1,ppt)
...
real(kind=wp),intent(in),optional::m0,m1

real(kind=wp),dimension(1:6),intent(out)::ele
real(kind=wp),intent(out),optional::ppt
...
real(kind=wp)::cappaq,a,e,incl,om,gom,mm,mmm,L2,rnorm,nnorm



!determine mass coefficients for orbit (units: solar masses)
    if(.not.present(m0)) then
       cappaq=1._wp
    else
       if(present(m1)) then
         cappaq=m0+m1
    ...
    ! attention: will change node for retrograde orbits!!!!
       ...
       if(incl.lt.1.d-13) then
        ...
        !non-coplanar, circular
         elseif (e.lt.1.d-13.and.incl.gt.1.d-13) then             
